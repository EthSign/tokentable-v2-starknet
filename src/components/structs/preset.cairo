...
/// A `Preset` is an unlocking schedule template that contains information that's shared across all recipients within a single round.
/// In this system, cliff unlocks are considered linear as well. This enables us to mix and match cliffs and linears at will, providing full customizability. 
/// Cliff waiting periods have a linear bip of 0 and cliff unlocking moments have a duration of 1 second.
...
/// * `stream`: If the tokens should unlock as a stream instead of a cliff at the end of a linear segment subdivision.
struct Preset {
