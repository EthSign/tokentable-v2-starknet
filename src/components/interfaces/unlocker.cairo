...
//!
//! The lightweight interface for TokenTableUnlockerV2(.5.x), which handles token unlocking and
//! distribution for TokenTable.

use starknet::{ContractAddress};
use tokentable_v2::components::structs::{preset::Preset, actual::Actual};
...
pub trait ITTUnlocker<TContractState> {
    ...
    /// * `recipient_id`: Emitted as an event reserved for EthSign frontend use. This parameter has
    /// no effect on contract execution.
    ///
    /// # Panics
    /// * `INVALID_PRESET_FORMAT`: If the preset is not formatted correctly (e.g. `linear_bips`
    /// fails to add up to `BIPS_PRECISION`)
    ...
    /// * `recipient_id`: Emitted as an event reserved for EthSign frontend use. This parameter has
    /// no effect on contract execution.
    /// * `batch_id`: Emitted as an event reserved for EthSign frontend use. This parameter has no
    /// effect on contract execution.
    ///
    /// # Panics
    /// * `PRESET_DOES_NOT_EXIST`: If `preset_id` does not exist.
    /// * `INVALID_SKIP_AMOUNT`: If the amount of tokens skipped is greater than or equal to the
    ...
    /// * `claim_to`: The destination address for the claimed tokens. Use Zeroable::zero() to claim
    ...
    /// * `TokensClaimed`
    fn delegate_claim(
        ref self: TContractState, actual_id: u256, recipient_id: u64, extraData: felt252,
    );
    ...
    /// * `wipe_claimable_balance`: Normally when canceling a schedule, calculation is performed to
    /// ensure the recipient will receive unlocked tokens until the moment of cancellation. This
    /// option prevents such calculation.
    /// * `recipient_id`: Emitted as an event reserved for EthSign frontend use. This parameter has
    /// no effect on contract execution.
    ///
    /// # Panics
    ...
    /// * `preset_linear_start_timestamps_relative`: Refer to
    /// `Preset.linear_start_timestamps_relative`.
    /// * `claim_timestamp_absolute`: The timestamp of when claim() is called. Must be in the
    /// future.
...
pub mod TTUnlockerEvents {
    ...
    pub struct ActualCreated {
        ...
        pub preset_id: felt252,
        pub actual_id: u256,
        pub recipient: super::ContractAddress,
    ...
    pub struct TokensClaimed {
        pub actual_id: u256,
        pub caller: super::ContractAddress,
        pub to: super::ContractAddress,
        pub amount: u256,
    ...
    pub struct ClaimingDelegateSet {
        pub from: super::ContractAddress,
        pub delegate: super::ContractAddress,
    }
}

pub mod TTUnlockerErrors {
    pub const INVALID_PRESET_FORMAT: felt252 = 'INVALID_PRESET_FORMAT';
    pub const PRESET_EXISTS: felt252 = 'PRESET_EXISTS';
    pub const PRESET_DOES_NOT_EXIST: felt252 = 'PRESET_DOES_NOT_EXIST';
    pub const INVALID_SKIP_AMOUNT: felt252 = 'INVALID_SKIP_AMOUNT';
    pub const NOT_PERMISSIONED: felt252 = 'NOT_PERMISSIONED';
    pub const GENERIC_ERC20_TRANSFER_ERROR: felt252 = 'GENERIC_ERC20_TRANSFER_ERROR';
    pub const ACTUAL_DOES_NOT_EXIST: felt252 = 'ACTUAL_DOES_NOT_EXIST';
}
